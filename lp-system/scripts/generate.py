...
def months_without_balcony(row):
    """Mirror of months_without_interior for the balcony class: months whose
    override carries an interior fare but NO balcony fare, because the source
    publishes no balcony that month.

    Without this, a balcony-less month silently inherits the row default
    balcony and prints a fare that does not exist at source. Until 2026-09-22
    the only defence was clearing the row's price_balcony by hand, which
    withheld the lead-in on every OTHER month too (three rows lost a real,
    published balcony floor that way) and left validate.py permanently
    flagging 'price missing', so a genuine missing price became
    indistinguishable from a deliberate clear."""
    return {m for m, v in parse_overrides(row).items() if "i" in v and "b" not in v}
...
def tracking_events(page, final_tel, final_display):
    ...
    POS CONTRACT (vocabulary is fixed at five values by operator ruling:
    deal-card / topbar / callzone / phoneband / sticky). The listener walks
    POS IN ORDER and takes the first entry with ANY matching ancestor, so
    the deal-card selectors stay first: a call CTA inside a deal surface is
    a deal-card click wherever else it happens to sit.
      deal-card — '.scard'/'.pcard'/'.itin' cards, '.noresults' (the empty
        state replaces the cards and carries the same CTA), '.fares' (the
        itinerary page's fare table IS that page's deal module), and
        '.sheet-deal' (the finder's row/featured-slide bottom sheet is the
        deal card's mobile expansion, same conversion element).
      topbar  — the whole header surface: '.topbar' (thin utility strip)
        AND '.nav' (the sticky header CTA, markup nav.nav > a.cta). Before
        2026-09-22 only '.topbar' was mapped, so the far more prominent
        sticky header phone reported 'other' on every page.
      callzone — every dedicated "call us about this" prompt: the
        '.callzone' card, the in-section '.callrow' prompts, the inline
        '.cabin-panel'/'.ship-panel' ask-a-specialist CTAs, the hero CTA
        ('.hero', the primary above-fold call button) and the content
        bottom sheets '.sheet-day'/'.sheet-life', whose foot CTAs are the
        same contextual prompt as a callrow.
      phoneband / sticky — unchanged.
    DELIBERATE 'other': the footer legal-block phone line (generated by
    lp_footer_html). It is a small-print link, not a conversion surface,
    and the five-value vocabulary has no footer term; mapping it into one
    of the five would misattribute it. 'other' should therefore be exactly
    one link per page — any higher count is drift and must be audited.
...
var POS=[['.scard','deal-card'],['.pcard','deal-card'],['.itin','deal-card'],['.noresults','deal-card'],['.fares','deal-card'],['.sheet-deal','deal-card'],['.topbar','topbar'],['.nav','topbar'],['.callzone','callzone'],['.callrow','callzone'],['.cabin-panel','callzone'],['.ship-panel','callzone'],['.sheet-day','callzone'],['.sheet-life','callzone'],['.hero','callzone'],['.phoneband','phoneband'],['.sticky-cta','sticky']];
...
def bake(page, reg, today, used):
    ...
    if RATES_RE.search(html):
        ...
        no_bal = months_without_balcony(r)
        month_list = months_of(r, today)
        months = [{"m": m,
                   "i": None if m in no_int else ov.get(m, {}).get("i", i_def),
                   "b": None if m in no_bal else ov.get(m, {}).get("b", b_def)}
                  for m in month_list]
        page_obj = {
